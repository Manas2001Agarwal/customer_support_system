...
def self_query_retrieval_invoke_chain(query:str):
    ...
    context = self_query_retriever_obj.call_retriever(query)
...
if __name__ == "__main__":
    eval_dataset = pd.read_csv("/Users/mukulagarwal/Desktop/Projects/customer_support_system/rag_evaluation/rag-qa-test-data.csv")
    eval_dataset = Dataset.from_pandas(eval_dataset)
    #basic_qa_ragas_dataset = create_ragas_dataset(eval_dataset,naive_retrieval_invoke_chain)
    self_query_qa_ragas_dataset = create_ragas_dataset(eval_dataset,self_query_retrieval_invoke_chain)
    self_query_qa_ragas_dataset.to_csv("rag_evaluation/self_query_retrieval_rag_test_data.csv",index = False)